...
│
├── product/                       # 产品部(原有,通用化)
│   ├── product-manager/           # 产品经理(54 skills)
│   ├── product-director/          # 产品总监
│   └── mvp-designer.md            # MVP 设计师
│
├── marketing/                     # 营销部(原有)
│   ├── Product-EVENT/             # 增长、活动
│   ├── media-content/             # 内容、社交媒体
│   ├── fans-domain/               # 私域
│   └── product-sales/             # 电商、直播
│
=======
...
| **strategy**(战略) | 3+ | 战略、商业模式、行业 | 战略规划、商业分析 |
| **product**(产品) | 3 | PRD、路线图、需求 | 产品规划、需求管理 |
| **marketing**(营销) | 9+ | 增长、内容、电商、私域 | 营销活动、获客、品牌 |
| **finance**(财务) | 2 | 财务建模、融资 | 财务规划、融资路演 |
| **organization**(组织) | 2 | 架构、招聘 | 组织设计、人才规划 |
| **risk**(风险) | 1 | 风险识别、合规 | 风险评估、合规审查 |

完整能力清单见 [`config/capabilities.yaml`](config/capabilities.yaml)。

## 添加新 Agent(极简流程)

1. 在对应部门下创建 `xxx.md`,写好 frontmatter:
```yaml
---
name: XX 专家
description: 一句话描述
...
2. 在 [`config/capabilities.yaml`](config/capabilities.yaml) 中注册

3. 在 [`config/routing-rules.yaml`](config/routing-rules.yaml) 中加路由规则(可选)

**就这么简单**,无需改其他任何文件。

## 添加新部门

1. 创建 `departments/newdept/` 目录
2. 编写 agent `.md` 文件(参考 `tech/` 或 `research/` 模板)
3. 在 `capabilities.yaml` 注册所有新 agent
4. 在 `routing-rules.yaml` 添加对应规则

## 添加新工作流

1. 在 `templates/` 下创建 `my-flow.yaml`
2. 格式参考 `software-dev.yaml`
3. 引用方式:`按 my-flow 跑`
...
| **飞书智能伙伴** | 把 `capabilities.yaml` 导入机器人配置 |
